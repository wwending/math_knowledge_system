...
5. 不要使用 \\(...\\) 作为行内公式分隔符。
6. 不要使用 \\[...\\] 作为块级公式分隔符。
7. 保持题目原意不变，不改变题意，不要自行补充题目没有给出的条件、答案或解析。
8. 不得根据常见题型猜测原题，不得补全缺失条件，不得替换变量、点名、线段名、焦点编号或选项内容。
9. 不得把一个数学表达式改成另一个数学表达式；只允许做 OCR 文本保真整理和 LaTeX 格式标准化。
10. 不得删除选项；即使 C/D 等选项看似残缺，也必须尽量保留 A/B/C/D 原始结构。
11. 不确定的地方保留原 OCR 内容，不要按常见题型自行修正。
12. 严禁把 AF1 改成 AF2；严禁把 |F1A|·|F1B|/|AB| 改写成 1/|AF2| + 1/|BF2|。
13. 严禁把 m^2y + 6 改成 my + b 这类未经依据的表达式。
14. 中文题目保持中文表达。
15. 提取 3-5 个高中数学知识点标签。
16. 不解题，不证明，不分析，不输出推理过程。
17. 只修正 OCR，只规范 LaTeX，只返回 JSON。
{metadata_instructions}
