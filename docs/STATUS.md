...
## 2026-08-31 超级管理员账号治理与用户名公开注册（#147）

- 用户管理收口为 `super_admin` 独占；可代创建 `user/admin`、治理启停/角色/密码并即时修改持久化公开注册开关，普通 `admin` 保留其他既有管理能力但不能访问用户管理。
- 新账号使用 NFC、大小写敏感用户名和可选唯一昵称；历史手机号继续作为兼容登录标识。所有密码设置入口统一为 6～64 个可打印 ASCII 字符，首次登录强制改密流程移除。
- 公开注册状态由数据库负责，已有部署首次迁移继承环境变量、新安装默认开启；匿名注册具备可信 IP 成功/失败限流、成功与限流审计、并发唯一冲突收口。

## 2026-08-30 逐题作答区行数（#126）

- PaperItem 新增试卷展示设置 `response_line_count`：合法值 `0..24`，新建默认及历史迁移均为 6；正常 Paper PATCH 可与标题、说明、显示开关、排序、分值和增删题一次原子保存，但仍不可修改冻结内容。
- 浏览器草稿预览实时使用本地行数；每行固定 `8mm`，正数作答区另有 `4mm` 顶部间距且整体不可拆，0 行无额外空间。答案或解析以及旧 `answer_area_mode=none` 只在渲染时隐藏，不改持久值。
- 正式 HTML/PDF 继续只读服务端已保存 Paper；存在未保存草稿时前端禁止导出并提示先保存。旧 `after_each_question` 改为使用逐题持久行数，新默认高度由 50mm 调整为 48mm。
...
- 新增纯前端整题文档状态内核：支持文字块新增、修改、删除、上下移动、相邻合并、跨区段移动，以及在 fenced code、inline code、转义美元和行内/块公式之外识别保守段落边界后拆分或跨区段移动完整段落。稳定 UUID、每区段 50 块、题干非空和元数据边界在保存前校验。
- 保存通过 schema-v2 document PUT 一次提交三区段、元数据和 existing figure 声明，并携带 `expected_revision_no`；`409`、网络和校验失败均保留完整本地草稿。路由离开、浏览器前进后退和刷新统一保护未保存修改，保存或取消后恢复 `tab=bank`、`bank_q` 并一次性重开原题详情。
- 前端 `test:stage3-contract` 全链与生产构建已通过；构建仍有既有 chunk size warning。真实浏览器的响应式布局、离开确认和并发冲突人工 smoke 尚未执行。

## 2026-08-30 上传确认页保存全部检测配图（#129）

- 上传确认页和一页多题逐题确认均默认保留全部有效检测框；支持移动、缩放、逐个删除、清空和重置，不再提供“主图”选择或手工新增检测框。重叠框会高亮并阻止保存，零检测/检测降级明确提示可按纯文字题保存。
- Draft `save-to-bank` 新增 `figure_bboxes[]` 公开合同（旧单框字段暂兼容），最多 10 张；全部框先完成坐标、面积、重叠、体积校验，再从原始 SourceAsset 原子裁图并建立 QuestionFigure/revision 关联。单图继续投影 legacy 字段，多图不虚构主图。
- 初始 schema-v2 题干在文字块后创建一个图片区，配图按稳定阅读顺序和自然像素尺寸左上排列、自动换行并增加区域高度；不新增迁移、依赖、独立编辑路由或上传页手工补图能力。
...
- 详情页下方“题目内容 / 答案 / 解析”统一承担查看与实时草稿预览；放弃修改恢复已保存内容，保存后的 revision 与 `409` 草稿保留语义不变。
- 新增可独立执行的编辑状态行为测试；前端 Stage 3 contract、生产构建与 `git diff --check` 均通过。真实浏览器人工验收尚未执行。

## 2026-08-28 题库题目编辑与回收站生命周期（#116）

- 题库支持题干、答案、解析、知识点标签、题型和难度的编辑；有实际变化时追加不可变 `QuestionRevision`，无变化不创建 revision，并通过 `expected_revision_no` 检测并发冲突。
- 题目支持进入回收站、恢复和用户侧逻辑永久删除；回收站保留 30 天，过期在查询时隐藏。历史 `PaperItem` 快照、共享资产和上传文件不因题目生命周期操作而物理删除。
- 题库详情保留大型题目区域图，并提供 Markdown/LaTeX 编辑与实时预览；后续 #123 将预览统一到详情内容区域，移除了编辑器内重复图片和窄屏分页。题库页提供 active/回收站页签及删除、恢复、永久删除确认流程。
- 后端全量 pytest、compileall、迁移往返；前端 Stage 3 contract、loader 行为测试和生产构建均已通过。真实浏览器人工验收、Staging/生产迁移与部署 smoke 尚未执行。
...
- 已配置的 Demo 域名已使用 Host Caddy `2.11.4` 和 Let's Encrypt 托管证书提供 HTTPS：公网 IPv4 仅开放 `22/80/443`，Caddy 在 `80` 重定向 HTTPS、在 `443` 终止 TLS 并代理到 `127.0.0.1:8000`；`backend:8000` 与 `gotenberg:3000` 仅在 Docker 内部，宿主机 `8000/8080/3000` 不对公网开放，也没有公网 IPv6 `80/443` listener。
