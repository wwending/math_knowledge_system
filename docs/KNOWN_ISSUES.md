...
- 后续应将 `PDF_TEMP_DIR` 一并移出公开挂载，并为临时文件补充清理或鉴权通道；在此之前不得把 pdf_temp URL 当作受保护资源使用。
- 该目录不在 #44 验收范围内，本条目在收口后移除。
...
Issue #29 已补齐完整试卷草稿编辑：标题/描述、分值、题序、删除、从题库新增，以及题干/答案/解析均可原子保存到 Paper/PaperItem；编辑不会修改 Question 或 QuestionRevision。

影响：

- 当前组卷仍不包含智能组卷、拖拽排序、Word 导出或复杂排版，不应表述为完整智能组卷系统。
- 后续如果补智能组卷或拖拽排序，应继续保持小步推进，并避免影响 Draft flow、legacy recognize 和题库保存逻辑。
