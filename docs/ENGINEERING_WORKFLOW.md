...
Before write-mode provisioning or launch, the launcher opens an exclusive `FileStream` (`FileShare.None`) under the shared Git common-dir. Its SHA-256 key includes the canonical repository identity, common Git directory, and exact worktree path. The handle remains open for the entire Worker process lifetime, so a second launcher for the same target returns `BLOCKED`, while different worktrees use independent leases. The OS releases the handle when the launcher exits abnormally; the small lock file may remain, but stale file existence does not block a later exclusive open and requires no cleanup.
