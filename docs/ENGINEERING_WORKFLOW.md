...
`scripts/codex-issue-worker.ps1` is the thin local handoff layer between the Control Codex and a writable Worker Codex. It requires an explicit `-ControlPath` and verifies that path is the repository's primary permanent Control Checkout rather than inferring control ownership from the script path or current directory. It accepts explicit Issue, branch, dedicated worktree, base, repository identity, and task-prompt inputs; validates the complete linked-worktree inventory; resolves the base to an exact commit; safely creates or reuses the expected branch/worktree; and starts a new Worker process whose process cwd and Codex workspace are both the dedicated Task Worktree.
...
For `CREATE_BRANCH_AND_WORKTREE`, the launcher verifies the created branch HEAD equals `resolvedBaseSha`. For `ATTACH_EXISTING_BRANCH` and `REUSE`, the Issue-numbered branch, exact path mapping, and recognized working state establish reuse ownership; the existing HEAD is preserved and verified unchanged. Structured output and the Worker prompt distinguish `requestedBaseRef`, `resolvedBaseSha`, and the actual `branchHeadSha`; the launcher never resets, rebases, or moves an existing branch to match the requested base.

Before write-mode provisioning or launch, the launcher starts a tracked supervisor and places it in an outer Windows Job Object configured with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`. The supervisor must then establish and verify an inner kill-on-close Job, join it, and acquire the exclusive `FileStream` (`FileShare.None`) lease under the shared Git common-dir before it reports readiness. The launcher does not provision or release the Worker launch gate unless every step succeeds. The actual Worker and its descendants inherit the inner Job; the supervisor verifies the Worker membership before sending the prompt. Forced launcher termination therefore closes the outer Job and terminates the supervisor tree, while independent supervisor termination closes the inner Job and terminates surviving Worker descendants. The supervisor owns the lease for that same lifetime, so ownership cannot become available while its Worker tree survives.

The lease SHA-256 key includes the canonical repository identity, common Git directory, and exact worktree path. A second launcher for the same target returns `BLOCKED`, while different worktrees use independent leases. Normal or failed Worker completion exits the supervisor and releases the handle. The small lock file may remain after the handle closes, but stale file existence does not block a later exclusive open and requires no cleanup. If Job creation, nested assignment, membership verification, or lease acquisition fails, startup fails closed before the writable Worker receives its prompt.
