...
- **登录标识**：登录时提交的用户名；历史账号仍可使用其手机号。新用户名与全部历史手机号共享命名空间，大小写敏感。
- **公开注册**：匿名访客以用户名、可选昵称和密码创建已启用普通用户的能力；不会创建管理员。
- **公开注册开放状态**：`auth_settings` 中持久化的开关值。已有部署首次迁移继承 `PUBLIC_SIGNUP_ENABLED`，此后数据库是唯一事实来源；新安装默认开启。
- **代创建账号**：超级管理员在用户管理中创建 `user` 或 `admin`；创建 `super_admin` 必须使用独立的显式角色提升操作。
- **账号治理**：仅 `super_admin` 可查看用户、代创建、启停用、调整角色、重置密码及修改公开注册状态；`admin` 不具备这些权限。
