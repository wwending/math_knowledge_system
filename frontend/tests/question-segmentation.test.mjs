...
const previousJobs = [
  ...
  { id: 2, bbox: [0, 0.6, 1, 0.2], draftId: 102, status: 'draft_ready', content: '待确认题', confirmedFigureBboxes: [[0.1, 0.1, 0.3, 0.2], [0.5, 0.1, 0.2, 0.2]] },
]
...
assert.deepEqual(reused.find((job) => job.id === 2).confirmedFigureBboxes, previousJobs[1].confirmedFigureBboxes)
assert.notEqual(reused.find((job) => job.id === 2).confirmedFigureBboxes, previousJobs[1].confirmedFigureBboxes)
assert.notEqual(reused.find((job) => job.id === 2).confirmedFigureBboxes[0], previousJobs[1].confirmedFigureBboxes[0])
