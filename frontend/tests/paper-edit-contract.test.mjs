...
const contracts = [
  ...
  ['题目内容为不可变快照', 'PaperPanel does not explain immutable snapshot semantics'],
  ['在题库中查看源题', 'PaperPanel does not provide the source-question entry'],
  ['修改源题不会同步当前试卷', 'PaperPanel does not explain source changes are not synchronized'],
  ['PaperSectionSnapshot', 'PaperPanel does not render schema-v2 section snapshots'],
  ['show_answer', 'PaperPanel does not persist the answer display option'],
  ['show_analysis', 'PaperPanel does not persist the analysis display option']
]
...
for (const field of ['content_snapshot', 'answer_snapshot', 'analysis_snapshot']) {
  if (source.includes(`v-model="item.${field}"`)) failures.push(`PaperPanel must not edit frozen ${field}`)
}
