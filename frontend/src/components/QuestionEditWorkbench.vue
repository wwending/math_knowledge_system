<template>
  <div class="question-workbench">
    ...
    <section v-show="activeTab === '原图'" class="question-image-pane">
    ...
    <section v-show="activeTab !== '原图'" class="question-editor-pane">
      ...
      <div v-show="activeTab === '预览'" class="preview-pane">
...
const save = async () => { if (!dirty.value || saving.value) return; saving.value = true; try { const payload = { ...draft.value, expected_revision_no: props.question.current_revision_no }; const res = await axios.put(`${API_V1_BASE_URL}/questions/${props.question.id}`, payload); baseline.value = JSON.parse(JSON.stringify(draft.value)); emit('saved', res.data); ElMessage.success('题目已保存') } catch (error) { if (error.response?.status === 409) ElMessage.error('题目已被其他修改更新，请保留草稿后刷新版本'); else ElMessage.error(error.response?.data?.detail || '保存题目失败') } finally { saving.value = false } }
</script>
<style scoped>
...
@media (max-width: 760px) { .question-workbench { display:block } .workbench-tabs { display:flex; gap:8px; margin-bottom:12px } .question-image-pane { min-height:300px } .question-region-image { height:300px } }
</style>
