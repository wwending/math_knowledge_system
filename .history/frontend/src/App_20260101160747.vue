...
import { UploadFilled, User, Lock, Document } from '@element-plus/icons-vue' // 记得引入 Document 图标
import axios from 'axios'
import Cropper from 'cropperjs' // 引入 Cropper
...
const pdfPages = ref([])        // 存放 PDF 每一页的图片路径
...
const handlePdfChange = async (file) => {
  ...
  try {
    ...
    ElMessage.error('PDF 上传失败，请检查文件格式')
  } finally {
    loading.value = false
  }
}
...
// --- ♻️ 重构：通用识别函数 ---
// (把原来的 startRecognition 逻辑抽离出来，接收 file 参数)
...
// 修改原有的 startRecognition，让它调用 runRecognition
const startRecognition = async () => {
  if (!selectedFile.value) return
  loading.value = true
