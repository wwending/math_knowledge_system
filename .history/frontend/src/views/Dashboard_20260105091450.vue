...
const handleLogout = () => {
  // ⚠️ 注意核对：我们在 Login.vue 里存的是 'token' 还是 'access_token'？
  // 根据之前的步骤，我们在 Login.vue 里写的是 localStorage.setItem('token', ...)
  // 所以这里也要删 'token'。如果你的键名不一样，请保持一致。
  localStorage.removeItem('token') 
  
  // 清理历史数据（可选，防止切回来闪现旧数据）
  historyList.value = [] 
  
  // 🔥 核心：不再操作 loginVisible，而是直接踢回登录页
  router.push('/login')
