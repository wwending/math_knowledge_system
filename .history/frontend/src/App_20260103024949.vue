...
const smartLatexFix = (text) => {
  ...
  const placeholders = []
  // 匹配 $$...$$ 或 $...$ 或 \(...\)
  res = res.replace(/(\$\$[\s\S]+?\$\$|\$[\s\S]+?\$|\\\([\s\S]+?\\\))/g, (match) => {
  ...
  // 3. 🔨 更激进的包裹逻辑
  // 只要看到反斜杠开头的单词（比如 \text, \frac, \Delta），或者常见的数学符号，就尝试包裹
  // 排除掉常见的标点符号转义（如 \. \,）
  const mathPattern = /(\\[a-zA-Z]+(\{[^}]*\})?(\{[^}]*\})?|[\u0391-\u03C9]|\^|_\{[^}]+\})/g

  res = res.split('\n').map(line => {
    // 如果这一行已经被占位符填满了，就不处理
    if (line.trim().startsWith('__MATH_PLACEHOLDER_') && line.trim().endsWith('__')) {
      return line
    }

    // 针对残留的 LaTeX 命令进行包裹
    // 比如：求 \text{AB} 的值 -> 求 $\text{AB}$ 的值
    line = line.replace(mathPattern, (match) => {
      // 如果匹配到的内容太短（比如只是一个字母），且不是希腊字母，可能不需要包
      // 但为了保险，我们把带 \ 的都包上
      if (match.startsWith('\\')) {
        return ` $${match}$ `
      }
    ...
    // 修复断裂分数 (1 \n 16 \n 3)
    // 注意：这里需要配合多行模式，split后很难处理跨行。
    // 如果之前的后端修复生效了，这里其实可以简化。
...
/* 4. 修复历史记录列表的容器 (如果有单独的容器类名) */
.history-list {
  /* 确保列表本身不限制高度，让 el-main 负责滚动 */
  height: auto; 
  padding-bottom: 50px; /* 底部留白，防止被遮挡 */
}
</style>